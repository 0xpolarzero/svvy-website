...
async function buildRepoCard(config: RepoConfig): Promise<RepoCardData> {
  ...
  // GitHub repo info
  try {
    const repo = await fetchJson<GitHubRepoInfo>(
  ...
  // NPM info
  if (config.npm) {
    const npm = await fetchJson<NpmPackageInfo>(
      `https://registry.npmjs.org/${config.npm}`,
    );
...
export async function fetchRepoCards(
  configs: RepoConfig[],
